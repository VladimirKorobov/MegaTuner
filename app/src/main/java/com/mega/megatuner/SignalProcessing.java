...
import com.mega.megatuner.Filters.Filter;
import com.mega.megatuner.Utils.ArrayIndexComparator;
    ...
    private void GetMaximums()
    {
        ...
        try
        {
            ...
            if(frequencyControl && globalMax > 1)
            {
                ...
                {
                    fft.rk4Filter = new Filter.RK4(data.freqMax, (double)freqFactor / this.bitRate, 0.1);
                }
                else
                {
                    fft.rk4Filter.Init(data.freqMax, (double)freqFactor / this.bitRate, 0.1);
                }
                if(syncByMax && data.freqMax != syncFreq)
                {
                    if(fft.rk4FilterMax == null)
                    {
                        fft.rk4FilterMax = new Filter.RK4(syncFreq, (double)freqFactor / this.bitRate, 0.1);
                    }
                    else
                    {
                        fft.rk4FilterMax.Init(syncFreq, (double)freqFactor / this.bitRate, 0.1);
                    }
                }
            }
            else
